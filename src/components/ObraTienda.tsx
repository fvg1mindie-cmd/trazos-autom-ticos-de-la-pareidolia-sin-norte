...
export function ObraTienda({ obra }: { obra: Artwork }) {
  ...
  const hayImpresiones = obra.impresiones.length > 0;
  ...
  const asunto = `Consulta — ${obra.titulo} (${obra.catalogo})`;
  const cuerpo = [
    `Obra: ${obra.titulo} (${obra.catalogo})`,
    `Pieza: ${pieza === "original" ? "Original" : `Impresión ${obra.impresiones[escala]?.escala ?? ""}`}`,
    `Montaje: ${montajes.find((m) => m.id === montaje)?.titulo}`,
    `Total estimado: ${formatPrecio(total, obra.moneda)}`,
  ].join("\n");
    ...
    <section className="mt-10 rounded-2xl border border-border/70 bg-card/50 p-6">
      ...
      <div className="mt-7 flex flex-wrap items-center justify-between gap-4 border-t border-border/70 pt-5">
        ...
        <a
          ...
          className="rounded-lg bg-primary px-5 py-3 font-mono text-[11px] tracking-[0.25em] text-primary-foreground uppercase"
        >
          Reservar / consultar
        </a>
      </div>
